...
function loadBrands() {
    ...
    db.ref('brands').once('value')
        .then((snapshot) => {
            if (!snapshot.exists()) {
                brandGrid.innerHTML = '<p class="no-results">No brands found. Add your first brand!</p>';
                return;
            }
        ...
        .catch((error) => {
            ...
            brandGrid.innerHTML = '<p class="error">Error loading brands. Please try again later.</p>';
        });
}
...
function updateBrand(brandId) {
    ...
    db.ref(`brands/${brandId}`).update(brandData)
        .then(() => {
